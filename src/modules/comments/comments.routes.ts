  ...
  app.get('/admin/comments', {
    ...
    const [items, total] = await Promise.all([
      app.prisma.comment.findMany({
        ...
        take: query.pageSize,
        select: {
          id: true, content: true, createdAt: true, isDeleted: true,
    ...
    return adminCommentsPage(items.map((item) => ({
      ...
      // Prisma field is `content`; the admin API response keeps its existing
      // `body` field name so the API contract does not change.
      body: item.isDeleted ? '' : item.content,
      createdAt: item.createdAt.toISOString(),
      isDeleted: item.isDeleted,
      authorName: item.user?.displayName ?? item.user?.username ?? 'کاربر حذف‌شده',
